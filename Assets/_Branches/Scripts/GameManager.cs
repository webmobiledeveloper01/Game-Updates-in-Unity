...
public RewardUIController sds;
    ...
    void Start()
    {
        pauseManager.instance.DisableGameObject();
    ...
    public void myCustomDies(){

        GameState = GameState.Paused;


        Debug.Log("Death myCustomDiesTRIGGERED");


        if (GameState != GameState.GameOver)
        {
            reviveObject.SetActive(true);
               Debug.Log("DeathmyCustomDies TRIGGERED1");

        }
        else
        {
            reviveObject.SetActive(false);
               Debug.Log("DeathmyCustomDies TRIGGERED2");
    ...
    public void StartGame()
    {
        ...
        pauseManager.instance.EnableGameObject();
    }
    ...
    public void GameOver()
    {
        ...
        adSahil.instance.ShowInterstitialAd();
        pauseManager.instance.DisableGameObject();
    }
                        ...
                        else //Create gold
                        {
                            ...
                            if (probabilityToCreateGold <= coinFrequency)
                            {
                                ...
                                else
                                {
                                    if (lastListPositionOfBranch[indexToCreateGold] == forwarPos)
                                    {
                                        currentGold = (GameObject)Instantiate(cake, lastListPositionOfBranch[indexToCreateGold] + new Vector3(0, 0, 1f), Quaternion.identity);
                                        currentGold.transform.SetParent(currentTrunk.transform);
                                    }
                                    else if (lastListPositionOfBranch[indexToCreateGold] == backPos)
                                    {
                                    ...
                                    {
                                        currentGold = (GameObject)Instantiate(cake, lastListPositionOfBranch[indexToCreateGold] + new Vector3(0, 1f, 0), Quaternion.identity);
                                        currentGold.transform.SetParent(currentTrunk.transform);
                                    }
                                    else if (lastListPositionOfBranch[indexToCreateGold] == downPos)
                                    {
